...
# have to change the model filter!! and potentially the x and y values
  ...
  mapping = aes(x = -3.5, y =0.7, label = round(DevianceExp,2), group=covariate),
col='#F8766D'
)+
    geom_text(
  data    = results_gam%>%filter(Model=="GLORYs"),
  mapping = aes(x = -3.5, y = 0.4, label = round(DevianceExp,2), group=covariate),
 col='#00BFC4'
)+
  xlim(c(-4,4))+
#annotate("text", x= -1, y = 0.8,label="Dev Explained")+
  guides(text="none")+
  theme_bw()
partial_effects 
```
