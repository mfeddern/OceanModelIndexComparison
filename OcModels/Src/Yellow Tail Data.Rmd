...
library(scales)  # Optional for nicer printing
...
# Make sure the columns are numeric before scaling
season_MOM6_monthly.df <- season_MOM6_monthly.df %>%
  mutate(across(all_of(cols_to_scale), ~ as.numeric(.)))

# Scale the values (column-wise z-score)
scaled_df <- season_MOM6_monthly.df %>%
  mutate(across(all_of(cols_to_scale), scale))

# Pivot longer
Scaled_season_MOM6YL_datalong <- scaled_df %>%
  pivot_longer(
...
```{r}
plot_function_by_season("MLD_0_180","Summer",Scaled_season_MOM6YL_datalong, "Seasonality Distribution of MLD","value" )
```
...
Scaled_season_GLORY_monthly.df <- season_GLORY_monthly.df %>%
  ...
  ungroup() %>%
  mutate(
    value = scaled_value,           # overwrite value with scaled version
    source = "MOM6",
    scaled = TRUE,
    Season = case_when(
      month %in% 1:3 ~ "Winter",
...
GLORY_monthlycombined_models <- bind_rows(
  ...
  group_by(year,name, Season, variable, source, scaled) %>%
  summarise(value = mean(value, na.rm = TRUE), .groups = "drop")
...
GLORYplot_function_by_season <- function(variable_name, season_name, dataframe, plot_title, y_axis) {
  ...
  dataframe %>%
    filter(name == variable_name, Season == season_name) %>%
    group_by(year, source, scaled, variable, Season) %>%
    summarise(value = mean(value, na.rm = TRUE), .groups = "drop") %>%
    ggplot(aes(x = year, y = value, color = source)) +
    geom_point() + 
...
```{r}
GLORYplot_function_by_season("lst_90_180","Summer",Scaled_season_GLORY_monthly.df, "Seasonality Distribution of LST","value" )
```
...
```{r}
GLORYall_seasonal_function("lst_90_180", Scaled_season_GLORY_monthly.df, "GLORY Seasonality Distribution of LST","value" )
```


```{r}
GLORYall_seasonal_function("mld_0_90", Scaled_season_GLORY_monthly.df, "GLORY Seasonality Distribution of MLD","value" )
```
...
p1 <- MOM6all_seasonal_function("MLD_90_180", Scaled_season_MOM6YL_datalong, 
                                "MOM6 Seasonality Distribution of MLD", "value") +
  theme(legend.position = "none")  # remove legend

p2 <- GLORYall_seasonal_function("mld_0_90", Scaled_season_GLORY_monthly.df, 
