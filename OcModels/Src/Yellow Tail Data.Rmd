...
### Making it so GLORYs data matches the MOM6 variables 
...
facet_version_annual_plot_scaled <- Scaled_combined_models %>%
  mutate(
    source = ifelse(source == "MOM6", "MOM6-NEP10k", source),
    variable_plot_subtitle = case_when(
      variable == "Tcop" ~ "A) Tcop",
      variable == "Tpart" ~ "B) Tpart",
      variable == "MLDpart" ~ "C) MLDpart",
      variable == "CSTlarv" ~ "D) CSTlarv",
      variable == "LSTlarv" ~ "E) LSTlarv",
      variable == "MLDlarv" ~ "F) MLDlarv",
      variable == "CSTpjuv" ~ "G) CSTpjuv",
      variable == "LSTpjuv" ~ "H) LSTpjuv",
      variable == "MLDpjuv" ~ "I) MLDpjuv"
    )
  ) %>%
  ggplot(aes(x = year, y = value, color = source)) +
  geom_point() + 
  ...
  facet_wrap(~variable_plot_subtitle) +
  labs(color = ("Model")) +
  xlab("Year") +
...
ggsave("Figure2.png", plot = facet_version_annual_plot_scaled, width = 7, height = 6.2, dpi = 300)
ggsave("Figure2.pdf", plot = facet_version_annual_plot_scaled, width = 7, height = 6.2, dpi = 300)

# view result
facet_version_annual_plot_scaled
```




```{r Facet Wrap UnScaled plots, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=8}

facet_version_annual_plot_unscaled <- UnScaled_combined_models %>%
  mutate(
    source = ifelse(source == "MOM6", "MOM6-NEP10k", source),
    variable_plot_subtitle = case_when(
      variable == "Tcop" ~ "A) Tcop",
  ...
  geom_line() + 
  scale_x_continuous(breaks = seq(min(Scaled_combined_models$year), max(Scaled_combined_models$year), by = 10 )) +
  facet_wrap(~variable_plot_subtitle, scales = "free_y") +
  labs(color = ("Model")) +
  xlab("Year") +
  ylab("Unstandardized value") +
  theme_bw() +
  theme(legend.position = "bottom")

ggsave("Figure3.png", plot = facet_version_annual_plot_unscaled, width = 7.2, height = 6, dpi = 300)
ggsave("Figure3.pdf", plot = facet_version_annual_plot_unscaled, width = 7.2, height = 6, dpi = 300)

# view result
facet_version_annual_plot_unscaled
```
...
plot_list <- Scaled_combined_models %>%
  ...
  lapply(function(df) {
    ggplot(df, aes(x = year, y = value, color = source)) +
      ...
      labs(title = unique(df$variable), color = "Model") +
      theme_minimal()
  })
...
```{r Generate Individual Plots for Each Variable}
...
make_plot <- function(var_name) {
  ...
  ggplot(df, aes(x = year, y = value, color = source)) +
    ...
    geom_hline(yintercept = 0, color = "darkblue", linetype = 3, linewidth = 1) +  # Add a horizontal reference line at y=0
    labs(
      title = var_name,    # Set the plot title as the variable name
      color = "Model"  # Label for the color legend
    ) +
    scale_x_continuous(
      breaks = seq(min(df$year), max(df$year), by = 5)  # Set x-axis breaks every 5 years
    ) +
    theme_bw() +  # Use a clean black-and-white theme
    theme(
      panel.spacing = unit(1, "lines"),  # Add spacing between panels if used in facets
...
LSTPjuv <- make_plot("LSTpjuv")   # Juvenile LST
LSTlarv <- make_plot("LSTlarv")   # Larval LST
CSTpjuv <- make_plot("CSTpjuv")   # Juvenile CST
CSTlarv <- make_plot("CSTlarv")   # Larval CST
Tpart <- make_plot("Tpart")       # Particle temperature
Tcop <- make_plot("Tcop")         # Copepod temperature
MLDlarv <- make_plot("MLDlarv")   # Larval mixed layer depth
MLDpjuv <- make_plot("MLDpjuv")   # Juvenile mixed layer depth
MLDpart <- make_plot("MLDpart")   # Particle mixed layer depth
```

```{r All variables Facet Wrap Plot}
...
```{r Generate Individual Plots for Unscaled Each Variable}

### Function to create plots for each variable
make_plot <- function(var_name) {
  # Filter the data for the specific variable
  df <- filter(UnScaled_combined_models, variable == var_name)
...
```{r All variables Unscaled Facet Wrap Plot}
...
# Arrange plots in 3×3 grid
all_UnScaledvariables_plot <- ggarrange(
  plotlist = plot_list,
  ncol = 3,
  nrow = 3,
  labels = LETTERS[1:length(plot_list)],
...
# Add single axis labels
annotate_figure(
  all_UnScaledvariables_plot,
  left = text_grob("Standardized values", rot = 90, size = 14),
  bottom = text_grob("Year", size = 14)
)

print (all_UnScaledvariables_plot)
```

```{r CST Facet Wrap Plots}
...
```{r Scaled and Unscaled Plot Function}

### Function to create a combined faceted plot for multiple variables
# This function filters the input data for the specified variables, then generates a faceted ggplot
# showing each variable over time with points and lines. It adds a horizontal reference line at 0,
# custom x-axis breaks, and allows custom facet labels and a plot title.


make_scaled_plot <- function(data, vars, facet_labels, plot_title) {
  ...
  p <- data %>%
    ...
    geom_hline(yintercept = 0, color = "darkblue", linetype = 3, linewidth = 1) +
    scale_x_continuous(
      breaks = seq(min(data$year), max(data$year), by = 4)
    ) +
    facet_wrap(
...
CST_ScaledPlots <- make_scaled_plot(
  data = Scaled_combined_models,
  vars = c("CSTlarv", "CSTpjuv"),
  facet_labels = c(
    "CSTlarv" = "Larval (CSTlarv)",
    "CSTpjuv" = "Pelagic Juvenile (CSTpjuv)"
  ),
  plot_title = "Scaled Comparison of Cross Shelf Transport (CST)\nfor Northern Yellowtail: GLORYS vs MOM6"
)
...
CST_UnScaledPlots <- make_scaled_plot(
  data = UnScaled_combined_models,
  vars = c("CSTlarv", "CSTpjuv"),
  facet_labels = c(
    "CSTlarv" = "Larval (CSTlarv)",
    "CSTpjuv" = "Pelagic Juvenile (CSTpjuv)"
  ),
  plot_title = "UnScaled Comparison of Cross Shelf Transport (CST)\nfor Northern Yellowtail: GLORYS vs MOM6"
)
...
LST_ScaledPlots <- make_scaled_plot(
  data = Scaled_combined_models,
  vars = c("LSTlarv", "LSTpjuv"),
  facet_labels = c(
    "LSTlarv" = "Larval (LSTlarv)",
    "LSTpjuv" = "Pelagic Juvenile (LSTpjuv)"
  ),
  plot_title = "Scaled Comparison of Long-Shore Transport (LST)\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(LST_ScaledPlots)
```


```{r Unscaled LST Variables, echo=FALSE, message=FALSE, warning=FALSE}

LST_UnScaledPlots <- make_scaled_plot(
  data = UnScaled_combined_models,
  vars = c("LSTlarv", "LSTpjuv"),
  facet_labels = c(
    "LSTlarv" = "Larval (LSTlarv)",
  ...
  plot_title = "UnScaled Comparison of Long-Shore Transport (LST)\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(LST_UnScaledPlots)
```


```{r  Scaled Temperature Variables}
Temp_Scaledplots <- make_scaled_plot(
  data = Scaled_combined_models,
  vars = c("Tcop", "Tpart"),
  facet_labels = c(
    "Tcop" = "Copulation (Tcop)",
    "Tpart" = "Parturition (Tpart)"
  ),
  plot_title = "Scaled Comparison of Temperature (T)\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(Temp_Scaledplots)
```

```{r UnScaled Temperature Variables}

Temp_UnScaledplots <- make_scaled_plot(
  data = UnScaled_combined_models,
  vars = c("Tcop", "Tpart"),
  facet_labels = c(
    "Tcop" = "Copulation (Tcop)",
  ...
  plot_title = "UnScaled Comparison of Temperature (T)\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(Temp_UnScaledplots)
...
MLD_Scaledplots <- make_scaled_plot(
  data = Scaled_combined_models,
  vars = c("MLDlarv", "MLDpart", "MLDpjuv"),
  facet_labels = c(
    "MLDlarv" = "Larval (MLDlarv)",
    "MLDpart" = "Parturition (MLDpart)",
    "MLDpjuv" = "Pelagic Juvenile MLDpjuv)"
  ),
  plot_title = "Scaled Comparison of Mixed Layer Depth\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(MLD_Scaledplots)

```

```{r UnScaled MLD plots}

MLD_UnScaledplots <- make_scaled_plot(
  data = UnScaled_combined_models,
  vars = c("MLDlarv", "MLDpart", "MLDpjuv"),
  facet_labels = c(
    "MLDlarv" = "Larval (MLDlarv)",
  ...
  plot_title = "UnScaled Comparison of Mixed Layer Depth\nfor Northern Yellowtail: GLORYS vs MOM6"
)

print(MLD_UnScaledplots)
...
MLDTemp_plot <- Temp_Scaledplots + MLD_Scaledplots + 
...
```{r MOM6 Filtering Years & Seasons}
...
MOM6plot_function_by_season <- function(variable_name, season_name, dataframe, plot_title, y_axis) {
  ...
  levels <- c("GLORYS", "MOM6")  # consistent legend levels
  
  dataframe %>%
    filter(variable == variable_name, Season == season_name) %>%
    mutate(source = factor(source, levels = levels)) %>%  # enforce levels
    ...
    scale_x_continuous(breaks = seq(min(dataframe$year), max(dataframe$year), by = 5)) +
    scale_color_manual(
  values = c("GLORYS" = "red", "MOM6" = "#00BFC4"),
  limits = c("GLORYS", "MOM6"),
  drop = FALSE
) +
...
```
```{r}
MOM6all_seasonal_function("MLD_0_180",Scaled_season_MOM6YL_datalong, "MOM6 Seasonality Distribution of MLD","value" )
```
```{r}
MOM6all_seasonal_function("XV_0_180",Scaled_season_MOM6YL_datalong, "MOM6 Seasonality Distribution of MLD","value" )
```
...
```{r GLORY Filtering Years & Seasons}
...
```{r All MLD 0_180 Season Plots}


Combined_WinterMLD <- GLORYplot_function_by_season(
  "MLD_0_180", "Winter (Jan–Mar)", Season_combined.df,
  "MLD", "value"
)

Combined_SpringMLD <- GLORYplot_function_by_season(
  "T_0_180", "Spring (Apr–Jun)", Season_combined.df,
  "MLD", "value"
)
...
Combined_AutumnMLD <- GLORYplot_function_by_season(
  "YV_0_180", "Autumn (Oct–Dec)", Season_combined.df,
  "MLD", "value"
)
```
...
Combined_WinterMLD <- Combined_WinterMLD + labs(x = NULL, y = NULL) + 
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

Combined_SpringMLD <- Combined_SpringMLD + labs(x = NULL, y = NULL) + labs(title = "  MLD - Spring (Apr–Jun)") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

# Remove y-axis labels and ticks for right two plots
Combined_SpringMLD <- Combined_SpringMLD +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())

Combined_AutumnMLD <- Combined_AutumnMLD + labs(title = "   MLD - Autumn (Oct–Dec)") +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) 

ALLSeasons_ggarranged_MLD <- ggarrange(
  Combined_WinterMLD,
  Combined_SpringMLD,
  Combined_SummerMLD,
  Combined_AutumnMLD,
...
ALLSeasons_ggarranged_MLD <- annotate_figure(
  ALLSeasons_ggarranged_MLD,
  left = text_grob("Standardized Value", rot = 90, size = 14, face = "bold"),
  bottom = text_grob("Year", size = 14, face = "bold")
)

print(ALLSeasons_ggarranged_MLD)
...
make_all_seasons_plot <- function(var_name, data) {

  # Map season names to dataset variable codes
  season_map <- list(
    Winter = list(code = "T_0_180", title = "Winter (Jan–Mar)"),
    Spring = list(code = "T_0_180",   title = "Spring (Apr–Jun)"),
    Summer = list(code = "T_0_180",  title = "Summer (Jul–Sep)"),
    Autumn = list(code = "T_0_180",  title = "Autumn (Oct–Dec)")
  )
  
  # Helper to create a season plot
  make_season_plot <- function(code, title) {
    GLORYplot_function_by_season(code, title, data, var_name, "value")
  }
  
  # Create plots
  p_winter <- make_season_plot(season_map$Winter$code, season_map$Winter$title) +
    labs(x = NULL, y = NULL) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
  
  p_spring <- make_season_plot(season_map$Spring$code, season_map$Spring$title) +
    labs(x = NULL, y = NULL, title = paste(var_name, "-", season_map$Spring$title)) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
          axis.text.y = element_blank(), axis.ticks.y = element_blank())
  
  p_summer <- make_season_plot(season_map$Summer$code, season_map$Summer$title)
  
  p_autumn <- make_season_plot(season_map$Autumn$code, season_map$Autumn$title) +
    labs(title = paste(var_name, "-", season_map$Autumn$title)) +
    theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
...
```{r}
plot_temp <- make_all_seasons_plot("Temperature", Season_combined.df) +
  theme(
    strip.text = element_text(
      size = 12, 
      margin = margin(b = 6)   # adds space below the facet labels
    )
  )
print(plot_temp)
```
...
season_codes_MLD <- list(
  Winter = list(code = "MLD_0_180", title = "Winter (Jan–Mar)"),
  Spring = list(code = "MLD_0_180", title = "Spring (Apr–Jun)"),
  Summer = list(code = "MLD_0_180", title = "Summer (Jul–Sep)"),
  Autumn = list(code = "MLD_0_180", title = "Autumn (Oct–Dec)")
)

# For Cross Shelf Transport
season_codes_XV <- list(
  Winter = list(code = "XV_0_180", title = "Winter (Jan–Mar)"),
  Spring = list(code = "XV_0_180", title = "Spring (Apr–Jun)"),
  Summer = list(code = "XV_0_180", title = "Summer (Jul–Sep)"),
  Autumn = list(code = "XV_0_180", title = "Autumn (Oct–Dec)")
)

# For Long Shore Transport
season_codes_YV <- list(
  Winter = list(code = "YV_0_180", title = "Winter (Jan–Mar)"),
  Spring = list(code = "YV_0_180", title = "Spring (Apr–Jun)"),
  Summer = list(code = "YV_0_180", title = "Summer (Jul–Sep)"),
  Autumn = list(code = "YV_0_180", title = "Autumn (Oct–Dec)")
)
...
```{r}
make_all_seasons_plot <- function(var_name, season_codes, data) {
  ...
  # Common theme for subplot titles (adds top margin)
  title_theme <- theme(
    plot.title = element_text(margin = margin(t = 12))  # adjust spacing as needed
  )
  
  # Create plots with consistent formatting
  p_winter <- make_season_plot(season_codes$Winter$code, season_codes$Winter$title) +
    labs(x = NULL, y = NULL, title = season_codes$Winter$title) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    title_theme
  
  p_spring <- make_season_plot(season_codes$Spring$code, season_codes$Spring$title) +
    labs(x = NULL, y = NULL, title = season_codes$Spring$title) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
          axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
    title_theme
  
  p_summer <- make_season_plot(season_codes$Summer$code, season_codes$Summer$title) +
    labs(title = season_codes$Summer$title) +
    title_theme
  
  p_autumn <- make_season_plot(season_codes$Autumn$code, season_codes$Autumn$title) +
    labs(title = season_codes$Autumn$title) +
    theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
    title_theme
  ...
  # Annotate (main labels + overall title)
  annotate_figure(
    arranged,
    top    = text_grob(var_name, face = "bold", size = 16),
    left   = text_grob("Standardized Value", rot = 90, size = 14, face = "bold"),
    bottom = text_grob("Year", size = 14, face = "bold")
  )
}
...
```{r}
plot_mld <- make_all_seasons_plot("Mixed layer depth", season_codes_MLD, Season_combined.df)
print(plot_mld)
```

```{r}
plot_temp <- make_all_seasons_plot("Temperature", season_codes_Temp, Season_combined.df) 
print(plot_temp)
```

```{r}
plot_XV <- make_all_seasons_plot("Cross-shelf transport", season_codes_XV, Season_combined.df)
print(plot_XV)
```

```{r}
plot_YV <- make_all_seasons_plot("Alongshore transport", season_codes_YV, Season_combined.df)
print(plot_YV)
```

```{r}
# function for facet wrap version (rather than ggarrange) of seasonal plot
facet_version_season_plot <- function(season_df, var_df_name, var_plot_name) {
  season_df |>
  filter(newname == var_df_name) |>
  group_by(year, Season, source) |>
  summarize(mean_value = mean(value)) |>
  mutate(
    source = ifelse(source == "MOM6", "MOM6-NEP10k", source),
    season_plot_subtitle = case_when(
      Season == "Winter (Jan–Mar)" ~ "A) Winter (Jan-Mar)",
      Season == "Spring (Apr–Jun)" ~ "B) Spring (Apr-Jun)",
      Season == "Summer (Jul–Sep)" ~ "C) Summer (Jul-Sep)",
      Season == "Autumn (Oct–Dec)" ~ "D) Autumn (Oct-Dec)"
    )
  ) |>
  ggplot(aes(x = year, y = mean_value, color = source)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.8) +
  facet_wrap(. ~ season_plot_subtitle) +
  theme_bw() +
  labs(title = var_plot_name, y = "Standardized value", x = "Year", color = "Model") +
  theme(legend.position = "bottom")
}
```
...
```{r}
# MLD seasonal plot
facet_t_seasonal <- facet_version_season_plot(
  season_df = Season_combined.df, 
  var_df_name = "T_0_180",
  var_plot_name = "Temperature"
)
ggsave(filename = "Figure4.png", plot = facet_t_seasonal, width = 7, height = 5, dpi = 300)
ggsave(filename = "Figure4.pdf", plot = facet_t_seasonal, width = 7, height = 5, dpi = 300)
facet_t_seasonal
```

```{r}
# MLD seasonal plot
facet_xv_seasonal <- facet_version_season_plot(
  season_df = Season_combined.df, 
  var_df_name = "XV_0_180",
  var_plot_name = "Cross-shelf transport"
)
ggsave(filename = "Figure5.png", plot = facet_xv_seasonal, width = 7, height = 5, dpi = 300)
ggsave(filename = "Figure5.pdf", plot = facet_xv_seasonal, width = 7, height = 5, dpi = 300)
facet_xv_seasonal
```

```{r}
# MLD seasonal plot
facet_yv_seasonal <- facet_version_season_plot(
  season_df = Season_combined.df, 
  var_df_name = "YV_0_180",
  var_plot_name = "Alongshore transport"
)
ggsave(filename = "Figure6.png", plot = facet_yv_seasonal, width = 7, height = 5, dpi = 300)
ggsave(filename = "Figure6.pdf", plot = facet_yv_seasonal, width = 7, height = 5, dpi = 300)
facet_yv_seasonal
```

```{r}
# MLD seasonal plot
facet_mld_seasonal <- facet_version_season_plot(
  season_df = Season_combined.df, 
  var_df_name = "MLD_0_180",
  var_plot_name = "Mixed layer depth"
)
ggsave(filename = "Figure7.png", plot = facet_mld_seasonal, width = 7, height = 5, dpi = 300)
ggsave(filename = "Figure7.pdf", plot = facet_mld_seasonal, width = 7, height = 5, dpi = 300)
facet_mld_seasonal
```
...
```{r MOM6 All Season Plot Function}
...
cov1<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(Tcop),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(Tcop))



cov2<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(Tpart),
...
cor1<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(Tcop),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(Tcop))



cor2<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(Tpart),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(Tpart))
...
cov3<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDpart),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDpart))

cov4<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDlarv))

cov5<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDpjuv))

cov6<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(LSTlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(LSTlarv))

cov7<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(LSTpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(LSTpjuv))

cov8<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(CSTlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(CSTlarv))

cov9<- cov(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(CSTpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(CSTpjuv))
```
...
cor3<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDpart),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDpart))

cor4<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDlarv))

cor5<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(MLDpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(MLDpjuv))

cor6<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(LSTlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(LSTlarv))

cor7<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(LSTpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(LSTpjuv))

cor8<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(CSTlarv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(CSTlarv))

cor9<- cor(Full_Glory_Yellow_Tail.df%>%filter(source=="GLORYS")%>%select(CSTpjuv),
Full_MOM6_Yellow_Tail.df%>%filter(source=="MOM6")%>%select(CSTpjuv))


# Create data frame with named covariates
results_df2 <- data.frame(
  covariate = c("Tcop", "Tpart", "MLDpart", "MLDLarv", "MLDpjuv", "LSTlarv", "LSTpjuv", "CSTlarv", "CSTpjuv"),
  covariance = c(cov1, cov2, cov3, cov4, cov5, cov6, cov7, cov8, cov9),
  correlation = c(cor1, cor2, cor3, cor4, cor5, cor6, cor7, cor8, cor9)
)

# Sort the data frame by correlation in descending order
results_df2 <- results_df2[order(-results_df2$correlation), ]
...
results_df2 %>%
  ...
  gt() %>%
  tab_header(
    title = "Correlation Between GLORYS and MOM6 Covariates"
  ) 
...
###Do I need to change the name of model name to "GLORYS" or "MOM6"
...
for(i in 1:length(covariatenames)){
  for(j in 1:length(Modelnames)){
    gamout<-gamoutput(Modelnames[j], covariatenames[i], LongScaled_CombinedModels_Rec_Yellow_Tail.df)
...
```{r}
# add covariate subplot name to smooths_gam and results_gam
smooths_gam <- smooths_gam %>%
  mutate(
    Model = ifelse(Model == "MOM6", "MOM6-NEP10k", Model),
    covariate_plot_subtitle = case_when(
      covariate == "Tcop" ~ "A) Tcop",
      covariate == "Tpart" ~ "B) Tpart",
      covariate == "MLDpart" ~ "C) MLDpart",
      covariate == "CSTlarv" ~ "D) CSTlarv",
      covariate == "LSTlarv" ~ "E) LSTlarv",
      covariate == "MLDlarv" ~ "F) MLDlarv",
      covariate == "CSTpjuv" ~ "G) CSTpjuv",
      covariate == "LSTpjuv" ~ "H) LSTpjuv",
      covariate == "MLDpjuv" ~ "I) MLDpjuv"
    )
  )
...
partial_effects <- ggplot(smooths_gam) +  # Setup the plot with the fitted values
  facet_wrap(~ covariate_plot_subtitle, ncol = 3) +
  geom_line(aes(x = value, y = .estimate, col = Model), lwd = 1.25) + # Add estimated smooth
    geom_point(
      data = LongScaled_CombinedModels_Rec_Yellow_Tail.df %>%
      ...
      aes(x = value, y = Y_rec, col = source),
      alpha = 0.2
    ) + # Add your data points
  geom_text(
    data    = results_gam %>% filter(Model == "MOM6-NEP10k"),
    mapping = aes(x = -3.5, y =0.7, label = round(DevianceExp, 2), group = covariate_plot_subtitle),
    col='#F8766D'
  ) +
  geom_text(
    data    = results_gam %>% filter(Model == "GLORYS"),
    mapping = aes(x = -3.5, y = 0.4, label = round(DevianceExp, 2), group = covariate_plot_subtitle),
    col='#00BFC4'
  ) +
  xlim(c(-4,4)) +
  guides(text = "none") +
...
ggsave(filename = "Figure8.png", plot = partial_effects, width = 7, height = 6, dpi = 300)
ggsave(filename = "Figure8.pdf", plot = partial_effects, width = 7, height = 6, dpi = 300)
partial_effects
```
